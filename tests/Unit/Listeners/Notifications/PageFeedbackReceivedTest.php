    ...
    public function test_email_sent_to_global_admin_email()
    {
        ...
        Queue::assertPushed(NotifyGlobalAdminEmail::class,
            function (NotifyGlobalAdminEmail $email) use ($pageFeedback) {
                $this->assertEquals(config('hlp.global_admin.email'), $email->to);
                $this->assertEquals(config('hlp.notifications_template_ids.page_feedback_received.notify_global_admin.email'),
                    $email->templateId);
